 ...
 *       results in {@link NullPointerException}.
 *   <li><b>Entry lifetime:</b> Entries may be <i>eternal</i> (no expiration) or <i>TTL-bound</i>
 *       (expire after a configured time-to-live). Once expired, an entry behaves as absent.
 *   <li><b>Consistency:</b> Size and metrics values may be eventually consistent under concurrent
 *       access. The API does not define iteration or ordering guarantees.
 * </ul>
 *
...
public interface Cache<K, V> {
    ...
    /**
     * Stores eternal entry (no TTL). replaces the previous value and clears any prior TTL.
     *
    ...
    /**
     * Returns an Optional containing the value if present and not expired, otherwise
     * Optional.empty(). Implementations may update hit/miss metrics
     *
     * @param key of the entry
